...
impl Connection {
        ...
        match sock_ref.take(msg_len as u64).read(&mut recv_buf) {
            ...
            Err(e) => {
                ...
                if e.kind() == ErrorKind::WouldBlock {
                    ...
                    self.read_continuation = Some(msg_len as u16);
                    Ok(None)
                } else {
    ...
    fn read_message_length(&mut self) -> io::Result<Option<u16>> {
    ...
    fn write_message_length(&mut self, buf: &Rc<Vec<u8>>) -> io::Result<Option<()>> {
        ...
        // BigEndian::write_u16(&mut send_buf[2..3], len as u16);

        let mut send_buf = vec![];
        send_buf.write_u16::<LittleEndian>(0).unwrap();
        send_buf.write_u16::<LittleEndian>(len as u16).unwrap();
        send_buf.write_u16::<LittleEndian>(0).unwrap();
        send_buf.write_u16::<LittleEndian>(0).unwrap();
